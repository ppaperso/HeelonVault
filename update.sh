...
EXCLUDE_PATTERNS=(
    ...
    "export-csv"
    "tests"
    "data"           # Données de développement uniquement (NE JAMAIS écraser /var/lib)
    "logs"
    "update.sh.old"
)

# ⚠️  SÉCURITÉ: Ces fichiers NE DOIVENT JAMAIS être écrasés ou supprimés
PROTECTED_FILES=(
    "*.db"           # Bases de données SQLite
    "salt_*.bin"     # Fichiers de sel de cryptage
    "*.db-shm"       # Fichiers SQLite shared memory
    "*.db-wal"       # Fichiers SQLite write-ahead log
)
...
# Fonction pour créer le backup tar.gz (INCLUT TOUJOURS LE DOSSIER DATA)
create_backup() {
    local version="$1"
    ...
    # ⚠️  CRITIQUE: Vérifier que le dossier DATA existe AVANT le backup
    if [ ! -d "$DATA_DIR" ]; then
        error_exit "Le dossier de données $DATA_DIR n'existe pas! Impossible de faire un backup sécurisé."
    fi
    
    # Compter les fichiers de données pour validation
    local data_file_count=$(find "$DATA_DIR" -type f \( -name "*.db" -o -name "salt_*.bin" \) 2>/dev/null | wc -l)
    if [ "$data_file_count" -eq 0 ]; then
        error_exit "Aucun fichier de données trouvé dans $DATA_DIR! Backup impossible."
    fi
    echo "   🔐 Fichiers de données détectés: $data_file_count"
    
    # Calculer les tailles
    local install_size=$(du -sh "$INSTALL_DIR" 2>/dev/null | cut -f1)
    local data_size=$(du -sh "$DATA_DIR" 2>/dev/null | cut -f1)
    echo "   📊 Taille installation: $install_size"
    echo "   📊 Taille données: $data_size"
    echo ""
    
    # ⚠️  CRITIQUE: Créer l'archive INCLUANT LE DOSSIER DATA
    echo "   🔄 Compression en cours (installation + données)..."
    cd /
    tar -czf "$BACKUP_FILE" \
    ...
    # ⚠️  CRITIQUE: Vérifier que les données sont BIEN dans le backup
    echo "   🔍 Vérification de l'intégrité..."
    if ! tar -tzf "$BACKUP_FILE" > /dev/null 2>&1; then
        error_exit "Le backup est corrompu!"
    fi
    
    # Vérifier que les fichiers .db sont dans le backup
    local db_in_backup=$(tar -tzf "$BACKUP_FILE" 2>/dev/null | grep -c "\.db$" || echo "0")
    if [ "$db_in_backup" -eq 0 ]; then
...
apply_update() {
    ...
    # ⚠️  SÉCURITÉ: Vérifier qu'on ne va PAS toucher au dossier DATA
    if [ -d "$source/data" ]; then
        echo "   ⚠️  ATTENTION: Le dossier source contient un dossier 'data'"
        echo "   🔒 Ce dossier sera IGNORÉ (données de développement uniquement)"
        echo "   🔒 Les données de production dans $DATA_DIR ne seront JAMAIS modifiées"
        echo ""
    fi
    ...
    # ⚠️  DOUBLE VÉRIFICATION: Le dossier data est bien exclu
    if [[ ! $exclude_args =~ "--exclude=data" ]]; then
        error_exit "ERREUR CRITIQUE: Le dossier data n'est pas dans les exclusions!"
    fi
    
    # Copier les fichiers avec rsync (SANS --delete sur DATA_DIR)
    echo "   📂 Copie des fichiers de l'application..."
    echo "   🔒 Exclusions actives: ${EXCLUDE_PATTERNS[@]}"
    echo ""
    ...
    # ⚠️  VÉRIFICATION POST-COPIE: Les données sont toujours là
    if [ -d "$DATA_DIR" ]; then
        local data_files_after=$(find "$DATA_DIR" -type f \( -name "*.db" -o -name "salt_*.bin" \) 2>/dev/null | wc -l)
        if [ "$data_files_after" -eq 0 ]; then
            error_exit "ERREUR CRITIQUE: Les fichiers de données ont disparu après la mise à jour!"
        fi
        echo "   🔐 Vérification: $data_files_after fichiers de données préservés"
    fi
}
...
# ⚠️  CRITIQUE: Vérifier que les données existent et sont valides
if [ ! -d "$DATA_DIR" ]; then
    error_exit "Le répertoire de données $DATA_DIR n'existe pas. Installation corrompue?"
fi

# Compter les fichiers de données
DATA_FILES_BEFORE=$(find "$DATA_DIR" -type f \( -name "*.db" -o -name "salt_*.bin" \) 2>/dev/null | wc -l)
if [ "$DATA_FILES_BEFORE" -eq 0 ]; then
    error_exit "Aucun fichier de données trouvé dans $DATA_DIR. Installation corrompue?"
fi
echo "✅ Répertoire de données présent ($DATA_FILES_BEFORE fichiers)"

# Vérifier les permissions de lecture sur les données
if [ ! -r "$DATA_DIR" ]; then
    error_exit "Impossible de lire le répertoire de données $DATA_DIR"
fi
...
echo "   • Un backup COMPLET (app + données) sera créé en tar.gz"
echo "   • Le dossier $DATA_DIR sera PRÉSERVÉ"
echo "   • Aucune base de données ne sera écrasée ou supprimée"
echo "   • $DATA_FILES_BEFORE fichiers de données seront protégés"
echo ""
echo "   🔄 ACTIONS DE MISE À JOUR:"
echo "   1. Création du backup complet (installation + données)"
...
# ⚠️  VÉRIFICATION CRITIQUE: Les données sont toujours présentes et intactes
DATA_FILE_COUNT=$(find "$DATA_DIR" -type f 2>/dev/null | wc -l)
DATA_FILES_AFTER=$(find "$DATA_DIR" -type f \( -name "*.db" -o -name "salt_*.bin" \) 2>/dev/null | wc -l)

if [ "$DATA_FILES_AFTER" -ne "$DATA_FILES_BEFORE" ]; then
    echo ""
    echo -e "${RED}❌ ERREUR CRITIQUE: Le nombre de fichiers de données a changé!${NC}"
    echo "   Avant: $DATA_FILES_BEFORE fichiers"
    echo "   Après: $DATA_FILES_AFTER fichiers"
    echo ""
    echo "   RESTAUREZ IMMÉDIATEMENT LE BACKUP:"
    echo "   sudo tar -xzf $BACKUP_FILE -C /"
    error_exit "Intégrité des données compromise!"
fi

echo "✅ Fichiers de données présents: $DATA_FILE_COUNT"
echo "✅ Fichiers critiques intacts: $DATA_FILES_AFTER (inchangé)"

# Vérifier la taille des fichiers .db (ne doit pas être 0)
for db_file in "$DATA_DIR"/*.db; do
    if [ -f "$db_file" ]; then
        db_size=$(stat -c%s "$db_file" 2>/dev/null || echo "0")
        if [ "$db_size" -eq 0 ]; then
            error_exit "ERREUR: Le fichier $db_file est vide! Restaurez le backup immédiatement."
        fi
    fi
done
