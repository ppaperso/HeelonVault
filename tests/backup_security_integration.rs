...
async fn test_non_admin_cannot_export_backup() {
    ...
    let app_service = heelonvault_rust::services::backup_application_service::BackupApplicationServiceImpl::new(
        user_repo,
        backup_service,
    );
    ...
    let result = app_service.export_backup_secured(
        non_admin_id,
        db_path.as_path(),
        backup_path.as_path(),
    ...
    assert!(matches!(result, Err(AppError::Authorization(_))), 
        "Expected Authorization error, but got: {:?}", result);
}
...
async fn test_admin_can_export_backup() {
    ...
    assert!(result.is_ok(), "Admin export should succeed, but got error: {:?}", result);
}
...
async fn test_missing_user_returns_not_found() {
    ...
    assert!(matches!(result, Err(AppError::NotFound(_))), 
        "Expected NotFound error for missing user, but got: {:?}", result);
}
