...
impl ManageUsersDialog {
		...
		TVault: VaultService + Send + Sync + 'static,
	{
		...
		let create_button = gtk4::Button::with_label(crate::tr!("manage-users-create-button").as_str());
						...
						Ok(Ok(users)) => {
							for user in users {
								...
								let role_text = match user.role {
										UserRole::Admin => crate::tr!("manage-users-role-admin").to_string(),
										UserRole::User => crate::tr!("manage-users-role-user").to_string(),
								};
									let role = gtk4::Label::new(Some(role_text.as_str()));
								role.set_width_chars(10);
								role.set_halign(gtk4::Align::Start);
								let created = gtk4::Label::new(
									Some(user.updated_at.as_deref().unwrap_or("-")),
								);
								created.set_width_chars(20);
								created.set_halign(gtk4::Align::Start);

								let reset_btn = gtk4::Button::with_label(crate::tr!("manage-users-reset-password").as_str());
								...
								let delete_btn = gtk4::Button::with_label(crate::tr!("manage-users-delete").as_str());
								...
								toggle_btn.connect_clicked(move |_| {
									...
									std::thread::spawn(move || {
										let result = runtime_for_task.block_on(async move {
											admin_for_task
												.update_user_role(actor_user_id, target_user_id, role_for_task)
												.await
										});
										let _ = sender.send(result);
									});
								...
								delete_btn.connect_clicked(move |_| {
									let confirm = adw::MessageDialog::new(
										...
										Some(crate::tr!("manage-users-delete-confirm-title").as_str()),
										Some(crate::tr!("manage-users-delete-confirm-body").as_str()),
									);
									confirm.add_response("cancel", crate::tr!("common-cancel").as_str());
									confirm.add_response("delete", crate::tr!("manage-users-delete").as_str());
									...
									confirm.connect_response(Some("delete"), move |d, _| {
										...
										std::thread::spawn(move || {
											let result = runtime_for_task.block_on(async move {
												admin_for_task.delete_user(actor_user_id, target_user_id).await
											});
											let _ = sender.send(result);
										});
								...
								reset_btn.connect_clicked(move |_| {
									let reset_window = gtk4::Window::builder()
										.title(crate::tr!("manage-users-reset-window-title").as_str())
										.modal(true)
									...
									password_entry.set_placeholder_text(Some(crate::tr!("manage-users-new-password-placeholder").as_str()));
									...
									let apply = gtk4::Button::with_label(crate::tr!("manage-users-apply").as_str());
									...
									cancel.connect_clicked(move |_| reset_window_for_cancel.close());
											...
											let result = runtime_for_task.block_on(async move {
												admin_for_task
													.reset_user_password(
														actor_user_id,
														target_user_id,
														secrecy::SecretBox::new(Box::new(password.into_bytes())),
													)
													.await
											});
											let _ = sender.send(result);
...

		{
			...
			create_button.connect_clicked(move |_| {
				...
				username.set_placeholder_text(Some(crate::tr!("manage-users-username-placeholder").as_str()));
				let password = gtk4::PasswordEntry::new();
				password.set_placeholder_text(Some(crate::tr!("manage-users-password-placeholder").as_str()));
