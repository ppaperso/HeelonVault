...
impl MainWindow {
    ...
    fn build_header_license_badge(license_badge_text: &str) -> gtk4::Widget {
        if let Some(customer_name) = Self::professional_customer_name(license_badge_text) {
    ...
    fn reset_auto_lock_timer(
        ...
        let source_id =
            glib::timeout_add_local_once(Duration::from_secs(timeout_secs), move || {
    ...
    fn build_certification_menu_item(icon_name: &str, label: &str) -> gtk4::Button {
    ...
    fn show_certification_diagnostics_dialog(
        ...
        let engine_fingerprint = Self::engine_fingerprint();
        ...
        details_list.append(&Self::build_certification_status_row(
            "Licence",
        ...
        if status.is_certified_license && !status.signing_key_present {
            ...
            generate_key_button.connect_clicked(move |_| {
                match license_service.ensure_audit_key_exists() {
                    ...
                    Err(error) => Self::show_feedback_dialog(
                        &parent_window,
                        "Provisionnement impossible",
                        format!("Impossible de générer la clé de signature: {:?}", error).as_str(),
                    ),
                }
            });
        ...
        TVault: VaultService + Send + Sync + 'static,
    {
        ...
        import_button.connect_clicked(move |_| {
            ...
            chooser.connect_response(move |dialog, response| {
                ...
                let Some(master_key) = Self::snapshot_session_master_key(&session_for_response)
                else {
                    Self::show_feedback_dialog(
                        &window_for_response,
                        crate::tr!("profile-import-accept").as_str(),
    ...
    fn build_sidebar_panel() -> SidebarWidgets {
        ...
        let (audit_all_row, audit_all_label, audit_all_badge) = Self::build_audit_sidebar_row(
            crate::tr!("main-audit-all").as_str(),
            "view-grid-symbolic",
        );
    ...
    fn build_center_panel() -> CenterPanelWidgets {
        ...
        let empty_description =
            gtk4::Label::new(Some(crate::tr!("main-empty-description").as_str()));
            ...
            > = runtime_for_loader.block_on(async move {
                ...
                for item in items {
                    ...
                    let secret_value = match secret_result {
                        Ok(secret) => {
                            String::from_utf8(secret.secret_value.expose_secret().clone())
                                .unwrap_or_default()
                        }
                        Err(_) => String::new(),
                    };
                    ...
                    let (color_class, kind) = match item.secret_type {
                        ...
                        crate::models::SecretType::SshKey => {
                            ("secret-type-ssh", SecretKind::SshKey)
                        }
                        crate::models::SecretType::SecureDocument => {
