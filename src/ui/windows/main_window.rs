...
impl MainWindow {
		...
		TImport: ImportService + Send + Sync + 'static,
	{
		...
		if !certification_enabled {
			sidebar_panel.certification_menu_button.set_tooltip_text(Some(
				"Certifier & Exporter (licence Pro requise)",
			));
		}
		...
		*editor_launcher.borrow_mut() = Some(open_editor.clone());
		...
		let launch_signed_report: Rc<dyn Fn(i64)> = Rc::new({
			...
			move |days| {
				...
				glib::MainContext::default().spawn_local(async move {
					match receiver.await {
						Ok(Ok(report)) => {
							toast_overlay_for_result
								.add_toast(adw::Toast::new(
									format!(
		...
		diagnostics_menu_button.connect_clicked({
			...
			move |_| {
				certification_popover.popdown();
				Self::show_certification_diagnostics_dialog(&report_window, Arc::clone(&license_service));
			}
		});
	...
	fn show_certification_diagnostics_dialog(
		...
		details_list.append(&Self::build_certification_status_row(
			"Licence",
			if status.is_certified_license { "PRO" } else { "FREE" },
