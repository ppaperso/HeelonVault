    ...
    TAudit: AuditLogRepository + Send + Sync,
{
    ...
    async fn list_for_target(
        ...
        self.require_admin(requester_id).await?;
        self.audit_repo.list_for_target(target_type, target_id, limit).await
    }
}
...
mod tests {
    ...
    impl UserRepository for StubUserRepo {
        ...
        async fn resolve_username_for_login_identifier(&self, id: &str) -> Result<Option<String>, AppError> {
            Ok(self.users.lock().unwrap().values().find(|u| u.username == id).map(|u| u.username.clone()))
        }
    ...
    impl AuditLogRepository for StubAuditRepo {
        async fn append(
            &self, actor: Option<Uuid>, action: &AuditAction,
            target_type: Option<&str>, target_id: Option<&str>, detail: Option<&str>,
        ) -> Result<(), AppError> {
        ...
        async fn list_for_actor(&self, actor_id: Uuid, limit: u32) -> Result<Vec<AuditLogEntry>, AppError> {
            let guard = self.entries.lock().unwrap();
            Ok(guard.iter()
        ...
        async fn list_for_target(&self, tt: &str, tid: &str, limit: u32) -> Result<Vec<AuditLogEntry>, AppError> {
            let guard = self.entries.lock().unwrap();
            Ok(guard.iter()
                .filter(|e| {
    ...
    async fn admin_can_list_recent() {
        ...
        let audit_repo = StubAuditRepo::default();
        audit_repo.append(Some(admin_id), &AuditAction::TeamCreated, Some("team"), Some("t1"), None).await.unwrap();
        let svc = AuditLogServiceImpl::new(user_repo, audit_repo);
    ...
    async fn user_can_view_own_audit_log() {
        ...
        let audit_repo = StubAuditRepo::default();
        audit_repo.append(Some(user_id), &AuditAction::UserCreated, Some("user"), Some(&user_id.to_string()), None).await.unwrap();
        let svc = AuditLogServiceImpl::new(user_repo, audit_repo);
        let entries = svc.list_for_user(user_id, user_id, 10).await.expect("list_for_user");
        assert_eq!(entries.len(), 1);
    }
